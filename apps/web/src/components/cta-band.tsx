...
export function CtaBand() {
  ...
  const { ctaBandText, stats } = getHomeContent(locale);
  const pills = [
    { ...stats[0], icon: Sparkles, position: 'start-[6%] top-9 sm:top-10', delay: 0.15, swing: -8 },
    { ...stats[2], icon: GraduationCap, position: 'end-[8%] top-14 sm:top-16', delay: 0.3, swing: 6 },
    { ...stats[1], icon: Rocket, position: 'start-[14%] bottom-9 sm:bottom-11', delay: 0.45, swing: -6 },
  ];
  const ghostWords = Array.from({ length: 6 }, () => ctaBandText.toUpperCase());
  ...
  return (
    <section className="relative isolate overflow-hidden bg-gradient-to-br from-brand-700 via-brand-500 to-brand-hero py-24 sm:py-32">
      ...
      >
        <div className="flex w-max items-center gap-16 animate-marquee" style={{ animationDuration: '48s' }}>
          {[...ghostWords, ...ghostWords].map((word, i) => (
            <span
              key={i}
              className="text-[7rem] font-extrabold uppercase leading-none tracking-tight text-white/[0.07] rtl:leading-[1.3] sm:text-[9rem] lg:text-[10rem]"
            >
              {word}
            </span>
          ))}
      ...
      <div className="pointer-events-none absolute -start-20 -top-20 h-72 w-72 rounded-full bg-white/10 blur-3xl" />
      <div className="pointer-events-none absolute -bottom-24 -end-16 h-80 w-80 rounded-full bg-accent-300/25 blur-3xl" />
      ...
      <div className="relative z-10 mx-auto max-w-3xl px-4 text-center sm:px-6">
        ...
        >
          ...
          <h2 className="animate-shimmer bg-gradient-to-r from-white/60 via-white to-white/60 bg-clip-text text-4xl font-extrabold leading-tight text-transparent rtl:leading-[1.4] [background-size:200%_auto] sm:text-5xl">
            {ctaBandText}
          </h2>
        </motion.div>
      </div>
