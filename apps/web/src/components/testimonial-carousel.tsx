...
export function TestimonialCarousel() {
  ...
  const { testimonials, testimonialsHeading, testimonialsSubhead } = getHomeContent(locale);
  ...
  return (
    <section data-nav-theme="dark" className="relative overflow-hidden bg-gradient-to-br from-brand-700 via-brand-600 to-ink py-16 sm:py-24">
      ...
      <div className="pointer-events-none absolute -top-20 start-[10%] h-80 w-80 rounded-full bg-brand-300/20 blur-3xl" />
      <div className="pointer-events-none absolute -bottom-24 end-[8%] h-96 w-96 rounded-full bg-accent-300/15 blur-3xl" />
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        >
          <h2 className="text-3xl font-extrabold text-white sm:text-[32px]">{testimonialsHeading}</h2>
          <p className="mt-2 text-white/85">{testimonialsSubhead}</p>
        </motion.div>
        ...
        >
          <div className="overflow-hidden">
            <div
              ...
              style={{ transform: `translateX(${rtlSign * index * 100}%)` }}
            >
              {Array.from({ length: pages }).map((_, pageIdx) => (
                <div key={pageIdx} className="grid w-full shrink-0 gap-5 sm:grid-cols-2 lg:grid-cols-3">
                  {testimonials.slice(pageIdx * PER_VIEW_LG, pageIdx * PER_VIEW_LG + PER_VIEW_LG).map((item) => (
                    <motion.div
                      key={item.name}
                      whileHover={{ y: -8 }}
                    ...
                    >
                      ...
                      <p className="mt-4 flex-1 text-[15px] leading-relaxed text-white">{item.quote}</p>
            ...
            <div className="flex items-center gap-2">
              {Array.from({ length: pages }).map((_, i) => (
                <button
                  ...
                  type="button"
                  onClick={() => goTo(i)}
                  aria-label={t('testimonialCarousel.goToSlide', { n: i + 1 })}
                  className={cn(
